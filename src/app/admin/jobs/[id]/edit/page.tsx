...
export default function AdminEditJobPage() {
      ...
      .then(data => {
        if (data.job) {
          ...
          setForm({
            ...
            industry: j.industry || "",
            functionalArea: j.functionalArea || "",
            roleCategory: j.roleCategory || "",
            jobType: j.jobType || "FULL_TIME",
            workMode: j.workMode || "OFFICE",
            city: j.city || "",
            state: j.state || "",
            location: j.location || "",
            ...
            salaryMin: j.salaryMin ? String((j.salaryMin / 100000).toFixed(1)) : "",
            salaryMax: j.salaryMax ? String((j.salaryMax / 100000).toFixed(1)) : "",
            salaryHidden: j.salaryHidden || false,
            education: j.education || "ANY",
            skills: Array.isArray(j.skills) ? j.skills : [],
            vacancies: String(j.vacancies || 1),
            status: j.status || "ACTIVE",
            featured: j.featured || false,
            approvedByAdmin: j.approvedByAdmin || false,
            walkInDate: j.walkInDate ? new Date(j.walkInDate).toISOString().slice(0, 16) : "",
            walkInVenue: j.walkInVenue || "",
          })
          if (j.functionalArea) {
            const cat = ROLE_CATEGORIES.find(r => r.area === j.functionalArea)
  ...
  function handleCityChange(city: string) {
    ...
    setForm(f => ({ ...f, city, ...(autoState ? { state: autoState } : {}) }))
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload = {
        ...
        walkInDate: form.walkInDate && form.walkInDate.trim() ? new Date(form.walkInDate).toISOString() : null,
        walkInVenue: form.walkInVenue || null,
      }
  ...
  const inputCls = "w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-3d p-6">
          <h2 className="font-bold text-navy mb-4 text-base border-b pb-2">Job Status & Visibility</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center gap-6 pt-5">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.approvedByAdmin} onChange={e => setField("approvedByAdmin", e.target.checked)}
                  className="w-4 h-4 accent-primary" />
                <span className="text-sm text-gray-700 font-medium">Approved (Live)</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input required type="text" value={form.title} onChange={e => setField("title", e.target.value)}
                placeholder="e.g. Sales Manager – Banking Products" className={inputCls} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Industry *</label>
              <select required value={form.industry} onChange={e => setField("industry", e.target.value)} className={selectCls}>
            ...
            <div>
              ...
              <select value={form.functionalArea} onChange={e => setField("functionalArea", e.target.value)} className={selectCls}>
            ...
            <div>
              ...
              {availableRoles.length > 0 ? (
                <select value={form.roleCategory} onChange={e => setField("roleCategory", e.target.value)} className={selectCls}>
                  <option value="">Select Role</option>
                  {availableRoles.map(r => <option key={r} value={r}>{r}</option>)}
                </select>
              ) : (
                <input type="text" value={form.roleCategory} onChange={e => setField("roleCategory", e.target.value)}
                  placeholder="e.g. Relationship Manager" className={inputCls} />
              )}
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Job Type *</label>
              <select required value={form.jobType} onChange={e => setField("jobType", e.target.value)} className={selectCls}>
                {JOB_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Work Mode</label>
              <select value={form.workMode} onChange={e => setField("workMode", e.target.value)} className={selectCls}>
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-3d p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input required list="edit-cities" type="text" value={form.city}
                onChange={e => handleCityChange(e.target.value)}
                placeholder="e.g. Mumbai" className={inputCls} />
              <datalist id="edit-cities">{POPULAR_CITIES.map(c => <option key={c} value={c} />)}</datalist>
              {form.city && CITY_STATE_MAP[form.city] && (
                <p className="text-xs text-primary mt-1">✓ {CITY_STATE_MAP[form.city]}</p>
              )}
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">State *</label>
              <select required value={form.state} onChange={e => setField("state", e.target.value)} className={selectCls}>
            ...
            <div>
              ...
              <input type="text" value={form.location} onChange={e => setField("location", e.target.value)}
                placeholder={form.city ? `e.g. Vesu, ${form.city}` : "Area, City"} className={inputCls} />
            </div>
          </div>
        </section>
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-3d p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Min Exp (yrs)</label>
              <input type="number" min="0" max="30" value={form.experienceMin} onChange={e => setField("experienceMin", e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Max Exp (yrs)</label>
              <input type="number" min="0" max="40" value={form.experienceMax} onChange={e => setField("experienceMax", e.target.value)} className={inputCls} />
            </div>
            <div>
          ...
          <div className="mt-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.salaryHidden} onChange={e => setField("salaryHidden", e.target.checked)}
                className="w-4 h-4 accent-primary" />
              <span className="text-sm text-gray-600">Hide salary (show "As per market standards")</span>
            </label>
          </div>
        </section>
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-3d p-6">
          ...
          <div className="mb-4">
            ...
            <select value={form.education} onChange={e => setField("education", e.target.value)} className={selectCls + " max-w-xs"}>
        ...
        <section className="bg-amber-50 rounded-2xl border border-amber-200 p-6">
          <h2 className="font-bold text-navy mb-4 text-base border-b border-amber-200 pb-2">Walk-in Details (optional)</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Walk-in Date & Time</label>
              <input type="datetime-local" value={form.walkInDate} onChange={e => setField("walkInDate", e.target.value)}
                className="w-full px-4 py-2.5 rounded-xl border border-amber-200 text-sm focus:outline-none bg-white" />
            </div>
            <div>
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-3d p-6">
          ...
          <textarea rows={5} value={form.requirements} onChange={e => setField("requirements", e.target.value)}
            placeholder="List required qualifications, must-haves..."
            className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none" />
        </section>
